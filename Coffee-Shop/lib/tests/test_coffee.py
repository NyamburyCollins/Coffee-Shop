...
from lib.order import Order  # Fixed import
...
def test_orders_property(customer, coffee):
    ...
    assert len(customer.orders) == 1  # Fixed: removed ()
    assert order in customer.orders   # Fixed: removed ()

def test_coffees_property(customer, coffee):
    Order(customer, coffee, 5.0)
    Order(customer, coffee, 5.0)
    assert len(customer.coffees) == 1  # Fixed: removed ()
    assert coffee in customer.coffees  # Fixed: removed ()

def test_create_order(customer, coffee):
    order = customer.create_order(coffee, 6.0)
    assert order in customer.orders    # Fixed: removed ()
    assert order in coffee.orders     # Fixed: removed ()
